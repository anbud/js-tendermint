...
function (_EventEmitter) {
  ...
  function Client() {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Client).call(this)); // parse full-node URI
    ...
    if (wsProtocols.includes(protocol)) {
      ...
      _this.uri = "".concat(protocol, "//").concat(hostname, ":").concat(port, "/websocket");
      _this.call = _this.callWs;

      _this.connectWs();
    } else if (httpProtocols.includes(protocol)) {
      _this.uri = "".concat(protocol, "//").concat(hostname, ":").concat(port, "/");
